    ...
    switch(evt->event_id) {
        ...
        case HTTP_EVENT_ON_FINISH:
            ...
            if (output_buffer != NULL) {
                ...
                //cJSON_Delete(workflow_status);
                //cJSON_Delete(workflow_conclusion);


                //printf("Workflow status: %s\n", workflow_status->valuestring);
                //printf("workflow conclusion: %s\n", workflow_conclusion->valuestring);
                /*char *workflow_status_result = strdup(workflow_status->valuestring);
                char *workflow_conclusion_result = strdup(workflow_conclusion->valuestring);
                */
                /*if (strcmp("completed", workflow_status_result) == 0) {
                    if (strcmp("success", workflow_conclusion_result) == 0) {
                        build_state_http = 2;
                ...
                //free(workflow_status_result);
                //free(workflow_conclusion_result);
                free(output_buffer);
                output_buffer = NULL;
            }
            output_len = 0;
...
int get_workflows_github_test(const char *repo_name, const char *repo_owner, const char *repo_authorization) {
    ...
    esp_http_client_config_t config_get = {
        ...
        .transport_type = HTTP_TRANSPORT_OVER_SSL,
        .crt_bundle_attach = esp_crt_bundle_attach,
        .event_handler = client_event_get_handler_test,
        .buffer_size = 512,
    };
